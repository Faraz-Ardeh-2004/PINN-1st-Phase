...
class model():
    def __init__(self, ques_name, ini_num) :
        ...
        for index, row in data.iterrows():
            ...
            # Keys containing min/max are generally function values and need to be converted to float for computation
            if 'min' in key or 'max' in key:
                self.model_ini_dict[key] = float(value)

            # Keys containing 'num' are generally quantities
            elif 'num' in key or 'state' in key:
                self.model_ini_dict[key] = int(value)

            # Keys containing 'state' are int values used for boolean checks
            elif 'state' in key:
                self.model_ini_dict[key] = int(value)
        ...
        # Whether to record each step
        self.pace_record_state = self.model_ini_dict['pace_record_state']

        # Define the number of nodes
        self.node_num = self.model_ini_dict['node_num']

        self.coord_num = self.model_ini_dict['coord_num'] if 'coord_num' in self.model_ini_dict else self.model_ini_dict['input_num']  # For backward compatibility with old Config files
        ...
        # Set parameter space values in advance!!!!
        ...
        self.layer[1:1] = list(map(lambda x: x * self.node_num, self.hidden_layers_group))  # This is an insertion operation
        self.layer = list(map(int, self.layer))

        # Set the database index to be used
        ...
        # Total number of nodes on the computational domain boundary
        self.grid_node_num = self.model_ini_dict['grid_node_num']

        # Presence or absence of supervised values, identified directly by filename, no need to redefine
        self.monitor_state = True if 'inv' in self.ques_name or 'global' in self.ques_name else False

        # Presence or absence of regularization
        self.regular_state = self.model_ini_dict['regularization_state']

        # Whether it is a loaded model
        self.load_state = self.model_ini_dict['load_state']

        # Number of iteration step groups
        self.step_num = self.model_ini_dict['step_num'] if self.model_ini_dict['step_num'] < 10 else 1

        # Number of nodes used on boundaries
        self.bun_node_num = self.model_ini_dict['bun_node_num']

        # Number of nodes used for plotting
        self.figure_node_num = self.model_ini_dict['figure_node_num']

        # Whether to use knowledge distillation, identified directly by filename, not redefined here
        self.distill_state = True if 'distill' in self.ques_name else False
        # Knowledge distillation should also have forward and inverse problems, so the inv identifier cannot be removed

        # self.distill_state = self.model_ini_dict['distill_state'] if 'distill_state' in self.model_ini_dict else False

        # Since knowledge distillation uses PINN, provide model parameters used, which are the same as above
        if self.distill_state:
            # Display distillation status
        ...
        # Recording interval increases after a certain number of steps
        ...
        ## It was found that the model size is relatively small, so all are saved

        # Whether the student network directly learns the existing loaded network
        self.load_study_state = self.model_ini_dict['load_study_state'] if 'load_study_state' in self.model_ini_dict else False

        # Teacher and student training steps
        ...
        # K-value control, after adding this value, separate teacher output from observation loss
        self.k_value = float(self.model_ini_dict['k_value']) if 'k_value' in self.model_ini_dict else 0.0


        # This indicates whether to add the pressure parameter p in fluid flow, default is add
    ...
    def mesh_init(self):
        if self.coord_num == 3:
            ...
            # Computational points for flow problems are from the flow data
            ...
            # Subtract 0.2 from y values to align the coordinate axis
            self.y -= 0.2
            ...
            # In fact, after calculating the mesh points, we just need to add a dimension in front of them
            # Here all possible parameter combinations are generated
            if self.para_ctrl_add:
                ...
                # Convert each combination to torch.Tensor
    ...
    def net_b(self):
        ...
        if 'Flow' in self.ques_name:
            # First read the boundary data: x, y, p, u, v
            ...
            '''
            Compute based on boundary data with the following constraints:
            1. Inlet velocity u has a fixed value, v = 0, direct subtraction
            2. Top and bottom boundaries and wall velocities: u = v = 0
            3. Outlet pressure: p = 0
            '''
            # Subtract 0.2 from all y values to align the coordinate axis
            inlet_data[:,1] -= 0.2
            wall_data[:,1] -= 0.2
            outlet_data[:,1] -= 0.2
            cylinder_data[:,1] -= 0.2
            ...
            loss_b_in = ((self.net(xy_in)[:, 1:3] - uv_in)**2).mean()  # Inlet velocity u has fixed value, v = 0


            # Cylinder surface boundary
            xy_cylinder = torch.tensor(cylinder_data[:,0:2], requires_grad=True).float().to(device)

            loss_b_cylinder_uv = ((self.net(xy_cylinder)[:, 1:3])**2).mean()  # Cylinder surface velocity is 0

            # Add cylinder surface pressure loss term
            p_cylinder = torch.tensor(cylinder_data[:,2], requires_grad=True).float().to(device)
            if self.flow_p_add:
                # Cylinder surface pressure is fixed
            ...
            loss_b_wall = ((self.net(xy_wall)[:, 1:3])**2).mean()  # Top and bottom boundary velocities are 0
            ...
            # loss_b_out = ((self.net(xy_out)[:, 0] - p_out)**2).mean()  # Outlet pressure is 0
            loss_b_out = ((self.net(xy_out)[:, 0])**2).mean()  # Outlet pressure is 0

            # Cylinder surface coefficient is defined in Config file
            ...
            if 'Burgers' in self.ques_name:
                u_b_moni = -torch.sin(torch.pi * y_b)  # Burgers equation
                loss_b += torch.mean((u_b - u_b_moni)**2)

                if 'half' in self.ques_name:
                    # Only compute half, boundary condition y_down is -1
                    x_down = torch.linspace(self.x_min, self.x_max, self.bun_node_num, requires_grad=True).float().to(device).reshape([-1,1])
                    y_down = -torch.ones_like(x_down, requires_grad=True).float().to(device).reshape([-1,1])
                    u_down = self.net(torch.cat([x_down, y_down], dim=1))

                u_down_moni = torch.zeros_like(u_down)  # Burgers equation
                loss_b += torch.mean((u_down - u_down_moni)**2)

                u_up_moni = torch.zeros_like(u_up)  # Burgers equation
                loss_b += torch.mean((u_up - u_up_moni)**2)
         #     loss_b += torch.mean((u_up - u_up_moni)**2)

            elif 'Laplace' in self.ques_name:
                u_b_moni = (x_b**3 - 3*x_b*y_b**2)  # Laplace equation
                loss_b += torch.mean((u_b - u_b_moni)**2)
    ...
    def net_f(self):
        ...
        if 'Flow' in self.ques_name:
            ...
            # Equation loss, needs to be converted to scalar
            loss_f += torch.mean((eq0)**2) + torch.mean((eq1)**2) + torch.mean((eq2)**2)

            return loss_f


        u_x = torch.autograd.grad(u, self.x, grad_outputs=torch.ones_like(u), retain_graph=True, create_graph=True)[0]    # This is the complete formula for automatic differentiation, just copy it directly
        u_xx = torch.autograd.grad(u_x, self.x, grad_outputs=torch.ones_like(u_x), retain_graph=True, create_graph=True)[0]
        u_y = torch.autograd.grad(u, self.y, grad_outputs=torch.ones_like(u), retain_graph=True, create_graph=True)[0]
        u_yy = torch.autograd.grad(u_y, self.y, grad_outputs=torch.ones_like(u_y), retain_graph=True, create_graph=True)[0]
        u_yyy = torch.autograd.grad(u_yy, self.y, grad_outputs=torch.ones_like(u_yy), retain_graph=True, create_graph=True)[0]

        # Equation residual error
        if 'Burgers' in self.ques_name:
            ...
            for name, param in parameters_rgl:
                if 'weight' in name:
                    ...
                    # Calculate 2-norm row-wise
                        ...
                        # If lambda_vals is not provided, auto-generate it
                        lambda_vals = torch.linspace(1, 0.1, steps=sorted_row_norms.size(0)).to(device)

                        # Ensure the length of lambda_vals matches the number of rows
                        lambda_vals = lambda_vals[:sorted_row_norms.size(0)]

                        # Compute the GrOWL regularization term
                        loss_rgl += torch.sum(lambda_vals * sorted_row_norms)
        return loss_rgl

    # Supervised loss for known global field data (with analytical solution or data available)
    ...
    # Can determine the presence of supervised values by state.
    def net_d(self, mode = 'teacher'):
        loss_d = torch.tensor(0.).to(device)

        # Here adopt a temporary strategy of separating names
        ques_name = self.ques_name.split('_')[0]


        # Since we need to stack, first bring one in
        ...
        # Since these supervised values are already in meshgrid state when stored, no need for meshgrid here
        ...
        if mode == 'student':
            ...
            # Perform fusion here
            # First compute the teacher's value at this point
            if self.k_value > 0:
        ...
        # Standard PINN loss computation
    ...
    def net_teach(self, weight_teach = 1):
        ...
        if self.coord_num == 3:
            ...
            # Now need to distinguish coordinate points in the teacher model that already have observation values
            # Here we must call net_d before net_teach due to including self.input_monitor
            if self.k_value > 0:
                # Calculate whether each row of xy_cat is completely identical to any row of input_monitor
                ...
                for row in self.input_monitor:
                    ...
                    mask = mask & (~same)  # Only keep unequal rows
    ...
    def model_save(self, suffix:str ='', mode:str='teacher'):
        ...
        # Copy control parameters (Config contents)
        ...
        # Store time, only save at the last step
        if suffix == '':
            ...
            if not os.path.isfile(file_path):
                self.time_save.to_csv(self.save_desti + 'Clock time.csv', mode='a', index=False)    # mode='a' means append mode
            else:
